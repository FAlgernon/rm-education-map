...
function getIWContent(num, type) {
    ...
    if(type === 1) {
        ...
        mouse = false;
        contentString = '<div id="iw_container" style="font-family: Verdana, Arial, Helvetica, sans-serif">' +
            '<div class="iw_title">' + arrLocations[num].facility +  ' | ' + parseAddress(arrLocations[num].address).state + '</div>'  + 
            '<div class="iw-content">';
    }
    
    for(var i = 0; i < arrEvents.length; i++) {
        var positionName = arrEvents[i].location;
        var eventDate = new Date(arrEvents[i].date);
    ...
    if(type === 1) {
		contentString = contentString + '</ul></div><br/> <p class="clickInfo">Click the marker for more information</p></div> </div>';
	} else {
		contentString = contentString + '</div><div id="tt" title=""></div></div>';
	}
